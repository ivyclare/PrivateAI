...
def train_model_with_dp(model_base, loader_base, model_canary, loader_canary, criterion, optimizer_base, optimizer_canary, privacy_engine_base, epochs=1, device='cpu', clipping_value=1, input_shape=(3, 32, 32), gradient_canary=False, input_canary=False, qc=1, target_observations=5000):
    ...
    for epoch in range(epochs):
        ...
        for data, targets in loader_base:
            ...
            output_canary = model_canary(data_0)
            loss_canary = criterion(output_canary, targets_0)
...
def white_box_auditing(dataloader, model_function, target_observations, epochs_per_observation, lr, clipping_value, target_epsilon, target_delta, device='cpu', qc=1, gradient_canary=True, input_canary=False, input_shape=(1, 28, 28)):
    ...
    train_model_with_dp(model_base, loader_base, model_canary, loader_canary, criterion, optimizer_base, optimizer_canary, privacy_engine_base,epochs_per_observation, device, clipping_value, input_shape, gradient_canary, input_canary, qc, target_observations)
